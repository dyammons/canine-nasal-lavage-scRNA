...
#######   begin preprocessing   ######## <<<<<<<<<<<<<<
######################################## <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
outName <- "nasal_lavage_n8_log_canFam"

load10x(din = "../input_CanFam3.1/", dout = "../output/s1_cfam/", outName = outName, testQC = F,
                    nFeature_RNA_high = 9000, nFeature_RNA_low = 200, 
                    nCount_RNA_high = 125000, nCount_RNA_low = 100, 
...
seu.obj <- integrateData(din = "../output/s1_cfam/", dout = "../output/s2/", outName = outName, normalization.method = "LogNormalize",
