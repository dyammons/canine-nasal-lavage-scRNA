...
# tail -n +6 CanFam3.1.104.filtered.gtf | 
# awk '$3 == "gene" {print $0}' | 
# sed 's/[";]//g;' | 
# awk '{OFS="\t"; print $1, $4,$5,$14,$10}' | 
# sort > gene_position.txt
...
#run inferCNV
message("[", paste0(Sys.time(), "]", " INFO: initating inferCNV run on ", 
                    unique(seu.obj$orig.ident), " using ", pthread, " cores."))
matrix <- as.matrix(seu.obj@assays$RNA$counts)
ann <- as.data.frame(seu.obj$majorID, row.names = rownames(seu.obj@meta.data))

inferobj <- CreateInfercnvObject(
    raw_counts_matrix = matrix,
    annotations_file = ann,
    gene_order_file = gene.pos.sorted,
    ref_group_names = c("moMac")
)

outDir <- paste0("../output/inferCNV_Ind/", unique(seu.obj$orig.ident),"/")
dir.create(outDir)
